...
.footer{text-align:center;color:#64748b;font-size:12px;margin-top:12px}
...
<div class="wrap">
  ...
  <section id="game" class="card hidden">
    <div class="gameTop"><div class="stat"><span>Round</span><b id="round">1/20</b></div><div class="stat"><span>Score</span><b id="score">0</b></div><div class="stat timer"><span>Time</span><b>⏱ <span id="timer">15</span></b></div></div>
  ...
  <div class="footer">LDF GAME • 20 rounds • 15 seconds each • max 200 points per round</div>
</div>
<script>
const socket=io();
let timerInt=null,answered=false;
const $=id=>document.getElementById(id);function show(id){$(id).classList.remove("hidden")}function hide(id){$(id).classList.add("hidden")}
function playerName(){return $("name").value.trim()||"Player"}function msg(t){$("msg").textContent=t}function escapeHtml(s){return String(s).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}
function renderPlayers(players){$("players").innerHTML=players.map(p=>\`<div class="player"><span>\${escapeHtml(p.name)}</span><span class="muted">Ready</span></div>\`).join("")}
$("create").onclick=()=>socket.emit("createRoom",{name:playerName()});$("join").onclick=()=>socket.emit("joinRoom",{name:playerName(),code:$("room").value});$("start").onclick=()=>socket.emit("startGame");
...
function renderBoard(list){$("board").innerHTML=list.map(p=>\`<div class="rankLine"><span class="rankNum">#\${p.rank}</span><span class="rankName">\${escapeHtml(p.name)}</span><span class="rankScore">\${p.score}</span></div>\`).join("")}
...
io.on("connection",socket=>{
  socket.on("createRoom",({name})=>{const rc=roomCode();const r={host:socket.id,players:new Map(),started:false,round:0,order:[],current:null,answered:new Set(),timer:null,startedAt:0,roundActive:false};r.players.set(socket.id,{name:(name||"Host").trim().slice(0,24)||"Host",score:0});rooms.set(rc,r);socket.join(rc);socket.roomCode=rc;socket.emit("created",{code:rc});lobby(rc);});
  socket.on("joinRoom",({code:rc,name})=>{rc=(rc||"").trim().toUpperCase();const r=rooms.get(rc);if(!r)return socket.emit("errorMsg","Room not found.");if(r.started)return socket.emit("errorMsg","That game has already started.");const clean=(name||"Player").trim().slice(0,24);if(!clean)return socket.emit("errorMsg","Please enter your name.");if([...r.players.values()].some(p=>p.name.toLowerCase()===clean.toLowerCase()))return socket.emit("errorMsg","That name is already in use.");r.players.set(socket.id,{name:clean,score:0});socket.join(rc);socket.roomCode=rc;socket.emit("joined",{code:rc});lobby(rc);});
